...
public class TempChannel extends ListenerAdapter {
    ...
    public void startTempVoice(EmbedBuilder EmbedBuilder, ActionRow actionRow){
        createVoiceJoin();
        if (actionRow == null){
          // net.sta.guild.getTextChannelById(TempChannelPermission.getTextChannelId())).sendMessageEmbeds(EmbedBuilder.build()).queue();
        }else {
         //   net.sta.guild.getTextChannelById(TempChannelPermission.getTextCahnnelid()).sendMessageEmbeds(EmbedBuilder.build()).setActionRow((Collection<? extends ItemComponent>) actionRow).queue();
